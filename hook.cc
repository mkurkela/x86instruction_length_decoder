...
protect(void *address, bool writable)
{
  ...
  {
    printf("vm_protect(%p, %d, %d) failed: %d\n",
  ...
  {
    printf("mprotect(%p, %d, %d) failed: %d\n",
...
parse(const char *func, void *address, unsigned *patches)
{
  ...
  if (insns[0] == 0xe9)
  {
    ...
    {
      printf("%s (%p): hook trampoline already installed, ignoring\n",
		   func, address);
      return -1;
    }
    else
      printf("%s (%p): jump instruction found, but not a hook target\n",
		   func, address);
  }
  ...
  while (n < 5)
  {
    int length;
    unsigned patch;
    length = myInstruction.decodeInstruction(insns, n);
    if (!length) 
    {
      printf("%s (%p) + 0x%x: unrecognised prologue (found 0x%x 0x%x 0x%x 0x%x)\n",
		   func, address, insns - (unsigned char *) address,
		   insns[0], insns[1], insns[2], insns[3]);
      return -1;
    }
    if ((patch = myInstruction.getPatch()) != 0)
    {
      *patches = patch;
      patches++;
    }
  ...
  {
    printf("%s (%p): mfctr can't be instrumented\n", func, address);
    return -1;
  }
	     ...
	     void **trampoline)
{
  ...
  if (version)
    printf("%s/%s (%p): instrumenting %d bytes into %p\n",
		 function, version, sym, prologue, tramp);
  else
    printf("%s (%p): instrumenting %d bytes into %p\n",
